...
class StockDataManager {
    static isCacheItemValid(symbol, cache, validity) {
    ...
    static async statsPeersInsidersAndCompany(symbol) {
        const combinedData = {};
    ...
    static async newsAndSocial(symbol) {
        const combinedData = {};
    ...
    //third tab
    static async financials(symbol) {
        const combinedData = {};
        ...
        if (cacheData) {
            ...
            return StockDataManager.getEarningsForSymbol(symbol, 8)
                ...
                return StockDataManager.getLatestFinancialDataForSymbol(symbol, StockDataManager.stockDao.incomeCollection, FMPService_1.default.incomeEndpoint, "quarter", 4);
            }).then(incomes => {
                combinedData["incomes"] = incomes;
                return StockDataManager.getLatestFinancialDataForSymbol(symbol, StockDataManager.stockDao.annualIncomeCollection, FMPService_1.default.incomeEndpoint, "annual", 4);
            }).then(annualincomes => {
                combinedData["incomesAnnual"] = annualincomes;
                return StockDataManager.getLatestFinancialDataForSymbol(symbol, StockDataManager.stockDao.cashFlowCollection, FMPService_1.default.cashFlowEndpoint, "quarter", 4);
            }).then(cashflow => {
                combinedData["cashflows"] = cashflow;
                return StockDataManager.getLatestFinancialDataForSymbol(symbol, StockDataManager.stockDao.annualCashFlowCollection, FMPService_1.default.cashFlowEndpoint, "annual", 4);
            }).then(cashflowAnnual => {
                combinedData["cashflowsAnnual"] = cashflowAnnual;
                return StockDataManager.getLatestFinancialDataForSymbol(symbol, StockDataManager.stockDao.balanceSheetCollection, FMPService_1.default.balanceSheetEndpoint, "quarter", 4);
            }).then(balanceSheets => {
                combinedData["balanceSheets"] = balanceSheets;
                return StockDataManager.getLatestFinancialDataForSymbol(symbol, StockDataManager.stockDao.annualBalanceSheetCollection, FMPService_1.default.balanceSheetEndpoint, "annual", 4);
    ...
    static async analysts(symbol) {
        ...
        if (cacheData) {
            ...
            let bptot = tipranksData === null || tipranksData === void 0 ? void 0 : tipranksData.bestPriceTargetsOverTime;
            ...
            combinedData["priceTargetsOverTime"] = tipranksData === null || tipranksData === void 0 ? void 0 : tipranksData.priceTargetsOverTime;
            combinedData["bestPriceTargetsOverTime"] = tipranksData === null || tipranksData === void 0 ? void 0 : tipranksData.bestPriceTargetsOverTime;
            let snap = StockDataManager.stockDao.getStockDocumentSnapshotForSymbol(symbol);
            if (snap) {
    ...
    static async getAllFreeDataForSymbol(symbol) {
    ...
    static getLatestFinancialDataForSymbol(symbol, collection, endpoint, period, limit) {
        ...
        return StockDataManager.stockDao.getMostRecentDocsFromSubCollectionForSymbol(symbol, collection, 16)
            ...
            if (!fundamentalDataDb || fundamentalDataDb.length === 0) {
                needToFetch = true;
                return FMPService_1.default.getFinancialDataForSymbol(symbol, period, endpoint, 16);
            }
            ...
            if (needToFetch) {
                ...
                for (let fd of fundamentalData) {
                    if (collection === StockDataManager.stockDao.cashFlowCollection) {
                        fd = FMPService_1.default.convertCashFlowToSimple(fd);
                    }
                    else if (collection === StockDataManager.stockDao.incomeCollection) {
                        fd = FMPService_1.default.convertIncomeStatementToSimple(fd);
                    }
                    else if (collection === StockDataManager.stockDao.balanceSheetCollection) {
                        fd = FMPService_1.default.convertBalanceSheetToSimple(fd);
                    }
    ...
    static getEarningsForSymbol(symbol, limit) {
        let needToFetch = false;
        return StockDataManager.stockDao.getMostRecentDocsFromSubCollectionForSymbol(symbol, StockDataManager.stockDao.earningsCollection, limit).then((earningsDB) => {
            if (!earningsDB || earningsDB.length === 0) {
                needToFetch = true;
                return FMPService_1.default.getEarningsForSymbol(symbol, limit).then((earnings) => {
                    return StockDataManager.stockDao.batchSaveMultipleDocsInSubcollectionForSymbols(this.stockDao.stockCollection, this.stockDao.earningsCollection, 'EPSReportDate', {
                        [symbol]: earnings
                    });
                });
            }
    ...
    static getCompanyFieldForSymbol(symbol) {
        ...
        if (!data || !data.lastUpdated || ((Date.now() - data.lastUpdated) > 172800000)) { //48hr
            return FMPService_1.default.getCompanyProfileAndPeers(symbol).then(data => {
    ...
    static async getSocialSentimentForSymbol(symbol) {
    ...
    static async getNewsForSymbol(symbol) {
        ...
        if (cacheItem && cacheItem.lastUpdated && (Date.now() - cacheItem.lastUpdated) <= StockDataManager.newsCacheValidity) {
            ...
            let news = await FMPService_1.default.getNewsForSymbol(symbol, 20);
            news.lastUpdated = Date.now();
            StockDataManager.newsCache[symbol] = news;
            return news;
        }
    }
    static async getNewsSentimentForSymbol(symbol) {
        ...
        if (cacheItem && cacheItem.lastUpdated && (Date.now() - cacheItem.lastUpdated) <= StockDataManager.newsSentimentCacheValidity) {
            ...
            let news = await TipranksService_1.default.getNewsSentiment(symbol);
            if (news) {
                news.lastUpdated = Date.now();
    ...
    static getAdvancedStatsForSymbol(symbol) {
        return StockDataManager.stockDao.getMostRecentDocFromSubCollectionForSymbol(symbol, StockDataManager.stockDao.advancedStatsCollection)
            .then(advancedStats => {
            if (!advancedStats) {
                return FMPService_1.default.getAdvancedStatsForSymbol(symbol, "annual", 1).then(advancedStatsFMP => {
                    if (advancedStatsFMP.length > 0) {
                        StockDataManager.stockDao.saveDocInSubcollectionForSymbol(symbol, StockDataManager.stockDao.advancedStatsCollection, advancedStatsFMP[0].date, advancedStatsFMP[0]).then().catch();
                        return;
                    }
                });
            }
    ...
    static getPriceTargetForSymbol(symbol) {
        return StockDataManager.getLatestDataFromCollectionForSymbol(symbol, StockDataManager.stockDao.priceTargetCollection, StockDataManager.iexDataService.priceTargetEndpoint, 30, false, false);
    }
    //IEX
    ...
    static getRecommendationsForSymbol(symbol) {
        return StockDataManager.getLatestDataFromCollectionForSymbol(symbol, StockDataManager.stockDao.recommendationCollection, StockDataManager.iexDataService.recommendationsEndpoint, 999999, true, false);
    }
    //FMP on demand, no more than once every 1 week
    static async getInsidersForSymbol(symbol) {
        let needToFetch = false;
        let result = StockDataManager.stockDao.getStockDocumentFieldForSymbol(symbol, StockDataManager.stockDao.insidersField);
        if (!result || Utilities_1.default.countDaysBetweenDates(Date.now(), result.lastUpdated) > 7) {
            needToFetch = true;
            result = await FMPService_1.default.getInsiderSummaryForSymbol(symbol);
        }
        if (needToFetch) {
    ...
    //     return simplifiedData
    // }
    static getLatestDataFromCollectionForSymbol(symbol, collection, endpoint, expireyInDays, isIexArray, isIexBatch) {
        let needToFetch = false;
        return StockDataManager.stockDao.getMostRecentDocFromSubCollectionForSymbol(symbol, collection)
            .then(result => {
            if (!result || Utilities_1.default.countDaysBetweenDates(Date.now(), result.id) > expireyInDays) {
                needToFetch = true;
                if (isIexBatch) {
    ...
    static getLatestQuotesForSymbols(symbols, alsoGetSimplifiedCharts) {
        return QuoteService_1.default.getLatestQuotes(symbols, alsoGetSimplifiedCharts);
    }
    static async getCompanyInfoForAllSymbols() {
    ...
    static getTipranksTopAnalystsForSymbol(symbol) {
    ...
    static getStocktwitsPostsForSymbol(symbol) {
        return StocktwitsService_1.default.getPostsForSymbol(symbol).then(result => {
            const results = [];
    ...
    static getScoresForSymbols(symbols = []) {
        let fields = ["company", "scores"];
    ...
    static applyUserScoreSettingsForSymbol(userid, symbol) {
        let settings = {};
        let areSettingsDefault = false;
        return UserManager_1.default.getUserScoreSettings(userid).then(s => {
            settings = s;
            ...
            if (settings && (settings.disabled.length || Object.keys(settings.weightings).length)) {
                ...
                for (const scoreObj of scoreObjs) {
                    ...
                    for (const categoryKey of Object.keys(scoreObj.scores.categories)) {
                        const categoryValuesMap = scoreObj.scores.categories[categoryKey];
                        let sum = 0.0;
                        let numMetrics = 0;
                        for (const metricKey of Object.keys(categoryValuesMap)) {
                            const metricValue = categoryValuesMap[metricKey];
    ...
    static areSettingsDefault(settings) {
        if (!settings) {
            return true;
        }
    ...
    static applyUserScoreSettings(userid, symbols = []) {
        let settings = {};
        let areSettingsDefault = false;
        return UserManager_1.default.getUserScoreSettings(userid).then(s => {
            settings = s;
...
exports.default = StockDataManager;
StockDataManager.stockDao = StockDao_1.default.getStockDaoInstance();
StockDataManager.iexDataService = IexDataService_1.default.getIexDataServiceInstance();
StockDataManager.freeDataCache = {};
StockDataManager.freeDataCacheExpirey = 43200000; //12 hours
StockDataManager.allCompaniesCache = [];
StockDataManager.allCompaniesCacheValidity = 86400000; //24hours
StockDataManager.allCompaniesCacheLastUpdate = 0;
StockDataManager.firstCache = {}; //company logo peers
//"second cache" is technically the socialsentiment and news caches because the numbers refer to UI tabs
StockDataManager.thirdCache = {}; //financials
StockDataManager.fourthCache = {}; //analysts
StockDataManager.groupedCacheValidity = 43200000; //12 hours
StockDataManager.socialSentimentCache = {};
StockDataManager.socialSentimentCacheValidity = 3600000; //1hour
StockDataManager.newsCache = {};
StockDataManager.newsCacheValidity = 3600000; //1hour
StockDataManager.newsSentimentCache = {};
StockDataManager.newsSentimentCacheValidity = 3600000; //1hour
//# sourceMappingURL=StockDataManager.js.map