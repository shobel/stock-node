...
class AnalysisService {
    ...
    static async doAnalysis() {
        var _a;
        let start = Date.now();
        let docSnaps = AnalysisService.stockDao.getAllStockDocumentSnapshots();
        //growth scores
        ...
        const incomeStatements = StockDao_1.default.getStockDaoInstance().incomeGroupCache; //await AnalysisService.stockDao.getDocumentsFromCollectionGroupWithDateCriteria(AnalysisService.stockDao.incomeCollection, "id", cutOffDate5, ">")
        const cashFlows = StockDao_1.default.getStockDaoInstance().cashFlowGroupCache; //await AnalysisService.stockDao.getDocumentsFromCollectionGroupWithDateCriteria(AnalysisService.stockDao.cashFlowCollection, "id", cutOffDate5, ">")
        const earnings = StockDao_1.default.getStockDaoInstance().earningsCache; //await AnalysisService.stockDao.getDocumentsFromCollectionGroupWithDateCriteria(AnalysisService.stockDao.earningsCollection, "id", todayString, ">")
        const advanced = StockDao_1.default.getStockDaoInstance().advancedGroupCache; //await AnalysisService.stockDao.getDocumentsFromCollectionGroupWithDateCriteria(AnalysisService.stockDao.advancedStatsCollection, "id", cutOffDate5, ">")
        let end = Date.now();
        console.log(`ANALYSIS: ${(end - start) / 1000.0}s to get docs for all stocks`);
        const valuationCategoryArrays = {};
        ...
        for (snap of docSnaps) {
            ...
            let quote = (_a = QuoteService_1.default.quoteCache[symbol]) === null || _a === void 0 ? void 0 : _a.latestQuote;
            if (!quote) {
                continue;
            }
            ...
            if (quote.pe && Utilities_1.default.isValidNumber(quote.pe) && quote.pe >= 0) {
                marketMetrics.peRatio.push(quote.pe);
                symbolMetrics[symbol].peRatio = quote.pe;
            }
            if (quote.eps && Utilities_1.default.isValidNumber(quote.eps)) {
                marketMetrics.epsTTM.push(quote.eps);
                symbolMetrics[symbol].epsTTM = quote.eps;
            }
            if (company.lastDiv && Utilities_1.default.isValidNumber(company.lastDiv) && company.lastDiv >= 0) {
            ...
            if (bs && is) {
                ...
                if (Utilities_1.default.isValidNumber(roe)) {
            ...
            if (cf && bs) {
                ...
                if (Utilities_1.default.isValidNumber(cashflowDebt)) {
            ...
            if (pt && pt.currency === "USD") {
                ...
                if ((quote === null || quote === void 0 ? void 0 : quote.price) != null) {
                    latestPrice = quote.price;
                }
                else if ((quote === null || quote === void 0 ? void 0 : quote.previousClose) != null) {
                ...
                if (latestPrice) {
                    ...
                    if (Utilities_1.default.isValidNumber(pts)) {
                        marketMetrics.priceTargetScore.push(pts);
                        symbolMetrics[symbol].priceTargetScore = pts;
                    }
                }
            }
            if (rec && Utilities_1.default.isValidNumber(rec.ratingScaleMark)) {
            ...
            if (company && Utilities_1.default.isValidNumber(company.sharesOutstanding) && inst && Utilities_1.default.isValidNumber(inst.change)) {
        ...
        const sortedCompanyScores = Object.values(companyScores).sort((a, b) => a.overallScore - b.overallScore);
        const overallScores = Object.values(sortedCompanyScores).map((s) => s.overallScore);
        const industryMap = {};
        for (let i = Object.values(sortedCompanyScores).length - 1; i >= 0; i--) {
            const cs = Object.values(sortedCompanyScores)[i];
        ...
        return AnalysisService.stockDao.batchSaveFieldsInMultipleStockDocs(AnalysisService.stockDao.stockCollection, AnalysisService.stockDao.scoresField, companyScores, true);
    }
    ...
    // }
    /* requires sorted arr */
    static computeRankPercentile(value, arr) {
    ...
    static computePercentile(value, arr, lowerIsBetter) {
        if (!arr.length) {
            return 1;
        }
        arr.sort((a, b) => a - b);
    ...
    static computeGrowthForMetric(symbol, metric, arr, field, marketMetrics, symbolMetrics, calculateRate, limit, growthRateMetric = "") {
        if (arr && arr.length) {
            ...
            if (diffs.length) {
                ...
                if (Utilities_1.default.isValidNumber(avgDiff)) {
                    marketMetrics[metric].push(avgDiff);
                    symbolMetrics[symbol][metric] = avgDiff;
                }
                if (calculateRate) {
                    const slope = Utilities_1.default.calculateTrendlineSlope(diffs);
                    if (Utilities_1.default.isValidNumber(slope)) {
...
AnalysisService.stockDao = StockDao_1.default.getStockDaoInstance();
AnalysisService.chartDao = ChartDao_1.default.getChartDaoInstance();
AnalysisService.marketDao = MarketDao_1.default.getMarketDaoInstance();
AnalysisService.valuationMetrics = ["peRatio", "epsTTM", "priceToSales", "priceToBook"];
AnalysisService.futureMetrics = ["futureRevenueGrowth", "futureIncomeGrowth", "pegRatio", "epsNextQGrowth", "priceTargetScore", "recommendationScore"];
AnalysisService.pastMetrics = ["avgRevenueGrowth", "avgIncomeGrowth", "revenueGrowthRate", "incomeGrowthRate", "cashFlowGrowth", "profitMarginGrowth"];
AnalysisService.healthMetrics = ["debtToAssets", "debtToEquity", "returnOnEquity", "tutes", "cashflowDebt", "dividendYield"];
// public static technicalMetrics = ["trends", "pressure", "support", "strength", "momentum", "circuitBreaker"]
AnalysisService.lowerIsBetterMetrics = ["peRatio", "debtToAssets", "debtToEquity", "priceToSales", "priceToBool", "pegRatio", "priceFairValue", "recommendationScore"];
AnalysisService.variableNamesMap = {
